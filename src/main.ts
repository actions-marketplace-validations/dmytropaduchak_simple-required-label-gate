...
const MARKER = "<!-- simple-required-label-gate -->";
const NAME = "Simple Required Label Gate";
...
async function run(): Promise<void> {
  ...
  const requiredLabels = (core.getInput("required-labels") || "")
    ...
    .map((s) => s.trim())
    .filter(Boolean);
  const mode = (core.getInput("mode") || "all").toLowerCase();
  const labels = (github.context.payload.pull_request?.labels || [])
    .map((l: { name?: string }) => l.name || "")
    .filter(Boolean);
  const findings = scan(labels, requiredLabels, mode);
  ...
  const shouldFail =
    failOn === "high"
      ...
      : failOn === "medium"
        ...
        : false;
  if (shouldFail) core.setFailed(`simple-required-label-gate: ${findings.length} finding(s) (fail-on=${failOn})`);
